...
\begin{enumerate}
    ...
    \item 后端，负责将 Koopa IR 翻译为 RISC-V（RV32IM）。
\end{enumerate}
\subsection{主要数据结构}
\subsubsection{抽象语法树}

语法树的根是 \inlinec{TranslationUnit}（翻译单元），\inlinec{TranslationUnit} 实质上是由 \inlinec{Definition}（符号定义）构成的列表。\inlinec{Definition} 是一个元组，包含符号的标识符、初始化器和符号的类型。

\textit{初始化器}是一个可区分联合，分为：
\begin{itemize}
    \item \inlinerust{Function(Vec<Option<String>>, Block)}（函数），其中 \inlinerust{Vec<Option<String>>} 记录了各个参数的标识符，\inlinerust{Block} 存储了函数体；
...
\textit{初始化器}和\textit{类型}\textbf{共同决定}了一个符号。例如：
\begin{itemize}
...
\end{itemize}
\subsubsection{优化}
在前端，对表达式进行代数化简，如 \inlinec{w = (x - x) * (y + z);} 会直接化简为 \inlinec{w = 0;}；当 \inlinec{if} 或 \inlinec{while} 的条件表达式可以确定时（如 \inlinec{while (x - x)}），则生成的 IR 直接使用 \texttt{j} 指令。

生成汇编时，对于栈上数据的引用，当栈大小小于 2048 字节时，使用 \texttt{sp} 寄存器访问；栈大小在 2048 \~{} 4096 字节时，高地址的数据使用 \texttt{bp} 寄存器访问。
...
\subsection{解析器}
\subsubsection{第一次解析}
第一次解析中，使用 pest\footnote{\url{https://pest.rs}} 将代码处理为语法树。因为 pest 设计上的局限性，此处语法树使用的是 pest 中 \inlinerust{Pairs<Rule>} 的形式。此外，这里还没有对运算符优先级做处理，表达式仍然表现为 \inlinerust{Pair} 流的形式。
\subsubsection{第二次解析}
